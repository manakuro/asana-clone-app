...
import { scheduleOnRN } from 'react-native-worklets/src/threads';
...
export function Toast({
  ...
  useEffect(() => {
    ...
    reduceMotion, // If there's content, start directly with expanded state
    width, // Animate in compact toast
    translateY,
    title,
    scale,
    height,
    borderRadius,
    opacity,
    description, // CORRECTED LINE: Use withDelay to wrap withTiming
    contentOpacity,
    action,
  ]); // Re-run if the reduced-motion setting resolves after mount
  ...
  }, [id, onDismiss, reduceMotion, translateY, scale, opacity]);
...
export function ToastProvider({ children, maxToasts = 3 }: ToastProviderProps) {
  ...
  const addToast = useCallback(
    ...
    [maxToasts, generateId, dismissToast],
  );
