...
func (_q *TagQuery) QueryColor() *ColorQuery {
...
func (_q *TagQuery) FirstID(ctx context.Context) (id ulid.ID, err error) {
	var ids []ulid.ID
...
func (_q *TagQuery) FirstIDX(ctx context.Context) ulid.ID {
...
func (_q *TagQuery) OnlyID(ctx context.Context) (id ulid.ID, err error) {
	var ids []ulid.ID
...
func (_q *TagQuery) OnlyIDX(ctx context.Context) ulid.ID {
...
func (_q *TagQuery) IDs(ctx context.Context) (ids []ulid.ID, err error) {
...
func (_q *TagQuery) IDsX(ctx context.Context) []ulid.ID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *TagQuery) WithColor(opts ...func(*ColorQuery)) *TagQuery {
	query := (&ColorClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withColor; query != nil {
		if err := _q.loadColor(ctx, query, nodes, nil,
			func(n *Tag, e *Color) { n.Edges.Color = e }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedTaskTags {
		if err := _q.loadTaskTags(ctx, query, nodes,
			func(n *Tag) { n.appendNamedTaskTags(name) },
			func(n *Tag, e *TaskTag) { n.appendNamedTaskTags(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TagQuery) loadColor(ctx context.Context, query *ColorQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Color)) error {
	ids := make([]ulid.ID, 0, len(nodes))
	nodeids := make(map[ulid.ID][]*Tag)
	for i := range nodes {
		fk := nodes[i].ColorID
...
func (_q *TagQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withColor != nil {
			_spec.Node.AddColumnOnce(tag.FieldColorID)
		}
	}
...
func (_q *TagQuery) WithNamedTaskTags(name string, opts ...func(*TaskTagQuery)) *TagQuery {
	query := (&TaskTagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
