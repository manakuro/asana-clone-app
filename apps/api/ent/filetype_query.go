...
func (_q *FileTypeQuery) QueryTaskFiles() *TaskFileQuery {
...
func (_q *FileTypeQuery) FirstID(ctx context.Context) (id ulid.ID, err error) {
	var ids []ulid.ID
...
func (_q *FileTypeQuery) FirstIDX(ctx context.Context) ulid.ID {
...
func (_q *FileTypeQuery) OnlyID(ctx context.Context) (id ulid.ID, err error) {
	var ids []ulid.ID
...
func (_q *FileTypeQuery) OnlyIDX(ctx context.Context) ulid.ID {
...
func (_q *FileTypeQuery) IDs(ctx context.Context) (ids []ulid.ID, err error) {
...
func (_q *FileTypeQuery) IDsX(ctx context.Context) []ulid.ID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *FileTypeQuery) WithTaskFiles(opts ...func(*TaskFileQuery)) *FileTypeQuery {
	query := (&TaskFileClient{config: _q.config}).Query()
...
func (_q *FileTypeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*FileType, error) {
	...
	if query := _q.withTaskFiles; query != nil {
		if err := _q.loadTaskFiles(ctx, query, nodes,
			func(n *FileType) { n.Edges.TaskFiles = []*TaskFile{} },
			func(n *FileType, e *TaskFile) { n.Edges.TaskFiles = append(n.Edges.TaskFiles, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedTaskFiles {
		if err := _q.loadTaskFiles(ctx, query, nodes,
			func(n *FileType) { n.appendNamedTaskFiles(name) },
			func(n *FileType, e *TaskFile) { n.appendNamedTaskFiles(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *FileTypeQuery) loadTaskFiles(ctx context.Context, query *TaskFileQuery, nodes []*FileType, init func(*FileType), assign func(*FileType, *TaskFile)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(taskfile.FieldFileTypeID)
	}
	query.Where(predicate.TaskFile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(filetype.TaskFilesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_type_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *FileTypeQuery) WithNamedTaskFiles(name string, opts ...func(*TaskFileQuery)) *FileTypeQuery {
	query := (&TaskFileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
